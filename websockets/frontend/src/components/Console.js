  ...
  return (
    <section className="absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] p-2 rounded bg-[grey] min-h-[600px] min-w-[800px]">
      <h1 className="bg-black p-2 mb-2">
        CONSOLE [{isConnected ? <span className="text-[green]">Online</span> : <span className="text-[red]">OFFLINE</span>}]
      </h1>
      <div className="bg-black h-[100%] min-h-[600px] p-2">
        ...
        <form className='flex'>
            <input placeholder='type ur message' className='outline-none bg-[grey] p-1 rounded w-[100%] resize-none' value={messageText} onChange={(e) => {
                setMessageText(e.target.value)
            }} />
            <button className='bg-[grey] p-1 rounded ml-2 outline-none' onClick={handleSendMessage}>
                send
            </button>
        </form>
      </div>
